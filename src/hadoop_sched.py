            ...
            if len(self.regular_tasks) != 0:
                for tid in self.regular_tasks:
                    ...
                    if len(self.available_nodes) != 0:
                        ...
                        if task["type"] == "map":
                            print("Assign Map job")
                            thread = threading.Thread(target=worker.execute_map_task, args=(tid,0))
                            self.worker_threads[node_id] = thread
                            thread.start()
                        elif task["type"] == "reduce":
                            print("Assign Reduce job")
                            thread = threading.Thread(target=worker.execute_reduce_task, args=(tid,0))
                            self.worker_threads[node_id] = thread
                            thread.start()
                        elif task["type"] == "copy":
                            print("Assign Copy job")
                            thread = threading.Thread(target=worker.execute_copy_task, args=(tid,0))
                            self.worker_threads[node_id] = thread
                            thread.start()
                        elif task["type"] == "sort":
                            print("Assign Sort job")
                            thread = threading.Thread(target=worker.execute_sort_task, args=(tid,0))
                            self.worker_threads[node_id] = thread
                            thread.start()
                    ...
                    for nid in self.node_progress_stats:
                        if self.node_progress_stats[nid]["progress_score"] < self.threshold and self.node_progress_stats[nid]["task_id"] != -1:
                            ...
                            if task["type"] == "map":
                                ...
                                thread = threading.Thread(target=worker.execute_copy_task, args=(tid,1))
                                self.worker_threads[node_id] = thread
                                thread.start()
                            elif task["type"] == "sort":
                ...
                if self.running_tasks[tid][1] == 1 or (tid in self.duplicate_tasks and self.duplicate_tasks[tid][1] == 1):
                        ...
                        with self.lock_tasks:
                            if self.running_tasks[tid][0]["type"] == "map":
                                self.task_id += 1
                                self.regular_tasks[self.task_id] = {"type": "copy"}
                                print("Generating new copy tasks")
                            elif self.running_tasks[tid][0]["type"] == "copy":
                                self.task_id += 1
                                self.regular_tasks[self.task_id] = {"type": "sort"}
                                print("Generating new sort tasks")
                            elif self.running_tasks[tid][0]["type"] == "sort":
                                self.task_id += 1
                                self.regular_tasks[self.task_id] = {"type": "reduce"}
                                print("Generating new Red tasks")
