...
def main():
    ...
    for i in range(len(timestamps_begin)):
        ...
        if dup1[i] == 1:
            ...
            c = "darkblue"
        plt.barh(nodes1[i], time_end - timestamps_begin[i], left=timestamps_begin[i], color=c, edgecolor="black", height=0.4)
        plt.text(timestamps_begin[i] + 0.004, nodes1[i], f"{tasks1[i]}", va='center', fontsize=12)
    # plt.xlabel("Timestamp")
    # plt.ylabel("Node ID")
    ...
    for i in range(len(cpy_timestamps_begin)):
        ...
        plt.text(cpy_timestamps_begin[i] + 0.004, cpy_nodes1[i], f"{cpy_tasks1[i]}", va='center', fontsize=12)
    ...
    for i in range(0, len(sort_timestamps_begin)):
        ...
        plt.text(sort_timestamps_begin[i]+0.004, sort_nodes1[i], f"{sort_tasks1[i]}", va="center", fontsize=12)
